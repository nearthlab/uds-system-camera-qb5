    ...
    ChiFeature2Messages*      pMessages)
{
    ...
    switch (pMessages->pFeatureMessages->messageType)
    {
        ...
        case ChiFeature2MessageType::MessageNotification:
            pFeatureGraph->m_featureGraphManagerCallbacks.ChiFeatureGraphProcessMessage(
                pMessages,
                pFeatureGraph->m_featureGraphManagerCallbacks.pPrivateCallbackData);
            break;
    ...
    if (foundErrorLink == TRUE)
    {
        ...
        else if (pOutputLinkData->linkType == ChiFeature2GraphLinkType::ExternalSink)
        {
            ...
            if (CDKResultSuccess == result)
            {
                ...
                if (CDKResultSuccess == result)
                {
                    // If we have notified the client with all results, set the usecase request object to Complete
                    BOOL allResultsDelivered =
                        ((TRUE == pUpstreamFeatureRequestObj->AreOutputsNotified(
                            pUpstreamFeatureRequestObj->GetCurRequestId())) &&
                          (TRUE == pUpstreamFeatureRequestObj->AreOutputsReleased(
                            pUpstreamFeatureRequestObj->GetCurRequestId())));

                    if (TRUE == allResultsDelivered)
                    {
                        CHX_LOG_INFO("%s All results are delivered", IdentifierString());
