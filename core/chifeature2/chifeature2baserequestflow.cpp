    ...
    if (CDKResultSuccess == result)
    {
        ...
        else
        {
                ...
                (static_cast<UINT32>(ChiFeature2Type::B2Y) == GetFeatureId()))
            {
                ...
                DumpInputMetaBuffer(&inputMetaInfo, 0);
                DumpInputImageBuffer(&inputBufferInfo, 0);
            }
    ...
    ) const
{
    ...
    std::vector<ChiFeature2BufferMetadataInfo> inputMetaInfo;
    ...
    if (CDKResultSuccess == result)
    {
        ...
        else if (stageId == 0)
        {
            ...
            inputBufferInfo.reserve(numFrames);
            inputMetaInfo.reserve(numFrames);
                ...
                if (NULL != pInputDependency)
                {
                    ...
                    if (CDKResultSuccess == result)
                    {
                        ...
                        for (UINT i = 0; i < numFrames; i++)
                        {
                            ...
                            CHX_LOG_INFO("GetBufferinfo RDI%d buffer %p meta handle %p",
                                         i, inputBufferInfo[i].hBuffer, inputMetaInfo[i].hBuffer);
                        }
                                    ...
                                    if (NULL != pInputDependency)
                                    {
                                        for (UINT8 bufferIndex = 0; bufferIndex < numFrames; ++bufferIndex)
                                        {
                                            if (ChiFeature2RequestState::Executing !=
                                                pRequestObject->GetCurRequestState(pRequestObject->GetCurRequestId()))
                                            {
                        ...
                        {
                            CHX_LOG_ERROR("Failed to submit request %d to session %d",
                                          pRequestObject->GetCurRequestId(), result);
                        }
                    }
                }
            }
