        ...
        if (NULL != m_pFeatureGraphSelector)
        {
            ...
            if ((NULL != m_pFeaturePoolManager) && (NULL != m_pFeatureThreadManager))
            {
                     ...
                     mapIterator != m_pFeatureGraphDescMapForConfig.end();)
                {
                    ...
                    featurePoolInputData.pUsecaseStreamTargetMap        = &m_usecaseStreamTargetMap;
    ...
    ChiFeature2UsecaseRequestObject* pRequestObject)
{
    ...
    {
        CHX_LOG_ERROR("NULL request object!");
...
// ChiFeature2GraphManager::ProcessFeatureGraphMessageCb
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2GraphManager::ProcessFeatureGraphMessageCb(
...
VOID ChiFeature2GraphManager::MapOutputPortToOutputStream(
    ...
    BOOL                                         isWithStreamMap)
{
    UINT8 numFeatureInstances          = pFeatureGraphDesc->numFeatureInstances;
    BOOL  mapFound                     = FALSE;
    BOOL is4KYUVOut                    = FALSE;
    std::vector<ChiStream*> mapStreams = outputStreams;

    CHX_LOG_VERBOSE("Number of framework streams to map %zu, isWithStreamMap %d",
        outputStreams.size(), isWithStreamMap);

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// isPortMappingConditionSatisfied
    ///
    /// @brief Checking various conditions to see if we have satisfied port-stream mapping condition
    ///        1. Check if feature port id from graph descriptor is same as pipeline port id and is imagebuffer port
    ///        2. Don't check for test stubs
    ///        3. If stream has physical camera field set then in addition to number 1, also check if physical camera id matches
    ///        for stream and feature instance.
    ///
    /// @param rFeatureGraphGlobalId    Port global id from graph
    /// @param rFeatureGlobalId         Port global id from pipeline
    /// @param rPipeline                Pipeline instance
    /// @param outputPortIdx            Output port index
    /// @param physicalCameraId         Physical camera id
    /// @param pChiStream               Output stream
    ///
    /// @return True if condition satisfied, False otherwise
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    auto isPortMappingConditionSatisfied = [](const ChiFeature2GlobalPortInstanceId& rFeatureGraphGlobalId,
                                              const ChiFeature2Identifier&           rFeatureGlobalId,
                                              const ChiFeature2PipelineDescriptor&   rPipeline,
                                              const UINT8                            outputPortIdx,
                                              const UINT32                           physicalCameraId,
                                              ChiStream*                             pChiStream)
    {
        return (((rFeatureGraphGlobalId.portId == rFeatureGlobalId)                                   &&
                 (NULL != rPipeline.pOutputPortDescriptor[outputPortIdx].pTargetDescriptor))          &&
                ((rFeatureGraphGlobalId.featureId != static_cast<UINT32>(ChiFeature2Type::STUB_RT))   &&
                 (rFeatureGraphGlobalId.featureId != static_cast<UINT32>(ChiFeature2Type::STUB_B2Y))) &&
                ((NULL == pChiStream->physicalCameraId)                                               ||
                 (0 == strlen(pChiStream->physicalCameraId))                                          ||
                 (ChxUtils::GetCameraIdFromStream(pChiStream) == physicalCameraId)));
    };
                ...
                for (UINT8 sessionIdx = 0; sessionIdx < pFeatureInstance.pFeatureDesc->numSessions; sessionIdx++)
                {
                    ...
                    for (UINT8 pipelineIdx = 0; pipelineIdx < session.numPipelines; pipelineIdx++)
                    {
                        ...
                        for (UINT8 OutputPortIdx = 0; OutputPortIdx < pipeline.numOutputPorts; OutputPortIdx++)
                        {
                            ...
                            for (UINT streamIdx = 0; streamIdx < mapStreams.size(); streamIdx++)
                            {
                                ChiFeature2PortIdToChiStreamMap map = {};
                                map.portId                          = featureGraphGlobalId;
                                map.pChiStream                      = mapStreams[streamIdx];
                                ...
                                ChiStream* pChiStream               = mapStreams[streamIdx];
                                const CHAR* pTargetName =
                                    pipeline.pOutputPortDescriptor[OutputPortIdx].pTargetDescriptor->pTargetName;

                                if (TRUE == isPortMappingConditionSatisfied(featureGraphGlobalId,
                                                                            featureGlobalId,
                                                                            pipeline,
                                                                            OutputPortIdx,
                                                                            physicalCameraId,
                                                                            pChiStream))
                                {
                                    if (isWithStreamMap)
                                    {
                                        for (auto mapIt: m_usecaseStreamTargetMap)
                                        {
                                            if ((featureGraphGlobalId.featureId == mapIt.first.featureId) &&
                                                (featureGraphGlobalId.instanceProps.instanceId == mapIt.first.instanceId))
                                            {
                                                if (!CdkUtils::StrCmp(pTargetName, mapIt.first.pTargetName))
                                                {
                                                    if (pChiStream == mapIt.second)
                                                    {
                                                        CHX_LOG_VERBOSE("Feature2Mapping: Streams Match for FeatureName: %s"
                                                                     "StreamMapTarget: %s Stream: %p PortTarget: %s Stream: %p",
                                                                    pFeatureName,
                                                                    mapIt.first.pTargetName,
                                                                    mapIt.second,
                                                                    pTargetName,
                                                                    pChiStream);
    ...
    ChiFeature2GraphDesc*            pFeatureGraphDescForRequest)
{
    ...
    BOOL isFeatureGraphForStreamMapping =
        m_pFeatureGraphSelector->IsFeatureGraphForStreamMapping(pFeatureGraphDescForRequest->pFeatureGraphName);

    MapOutputPortToOutputStream(pFeatureGraphDescForRequest,
                                outputStreams,
                                globalPortIdToChiStreamMapForOutput,
                                isFeatureGraphForStreamMapping);
...
CDKResult ChiFeature2GraphManager::Flush()
{
    ...
    m_isFlush    = TRUE;
    result       = m_pFeaturePoolManager->Flush(FALSE);
    m_isFlush    = FALSE;
    return result;
...
VOID ChiFeature2GraphManager::Destroy()
{
    ...
    m_usecaseStreamTargetMap.clear();
