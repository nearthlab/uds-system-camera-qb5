    ...
    Feature2WrapperCreateInputInfo* pCreateData)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        m_pSubmissionLock   = Mutex::Create();

        if ((NULL == m_pUROMapLock) || (NULL == m_pFGMLock) || (NULL == m_pSubmissionLock))
        {
            result = CDKResultENoMemory;
            CHX_LOG_ERROR("Cannot allocate URO lock %p, FGM lock %p or Submission lock %p",
                          m_pUROMapLock,
                          m_pFGMLock,
                          m_pSubmissionLock);
        }
    ...
    BOOL isForced)
{
    ...
    if (NULL != m_pSubmissionLock)
    {
        m_pSubmissionLock->Destroy();
        m_pSubmissionLock = NULL;
    }
        ...
        for (UINT32 i = 0; i < pExtSrcStreamData->numInputs; i++)
        {
            ...
            if (CDKResultSuccess == result)
            {
                ...
                result = pRequestInternalInputTBM->ImportExternalTargetBuffer(
                    fwkFrameNum + i, reinterpret_cast<UINT64>(pInternalStream), &inputBuffer);

                // Setup Buffer TBH
                hOutputTBH = pRequestInternalInputTBM->SetupTargetBuffer(fwkFrameNum + i);

                // Update status to ready
                pRequestInternalInputTBM->UpdateTarget(fwkFrameNum + i,
                ...
                if (NULL != pSrcMetadata)
                {
                    ...
                    result = pRequestInputMetadataTBM->ImportExternalTargetBuffer(
                        fwkFrameNum + i, m_metaClientId, pSrcMetadata);

                    // Setup Metadata TBH
                    hMetadataTBH = pRequestInputMetadataTBM->SetupTargetBuffer(fwkFrameNum + i);

                    // Update status to ready
                    pRequestInputMetadataTBM->UpdateTarget(fwkFrameNum + i,
                                                           m_metaClientId,
                                                           pSrcMetadata,
                                                           ChiTargetStatus::Ready,
                                                           NULL);
    ...
    if (NULL != pUsecaseRequestObject)
    {
        ...
        for (UINT32 j = 0; j < pResult->numOutputBuffers; j++)
        {
                ...
                (TRUE == m_pUsecaseBase->IsMultiCameraUsecase()))
            {
                ...
                // if physicalCameraId is NULL, it indicates that it is fusion stream
                // Otherwise, it is single zone yuv stream
                if (NULL != pResult->pOutputBuffers[j].pStream->physicalCameraId)
                {
                    UINT32 cameraId   = ChxUtils::GetCameraIdFromStream(pResult->pOutputBuffers[j].pStream);
                    UINT32 pipelineId = m_pUsecaseBase->GetPipelineIdFromCamId(cameraId);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID Feature2Wrapper::ProcessMessageCb(
    ...
    VOID*                       pPrivateCallbackData)
{
    ...
    else if (NULL != pMessageDescriptor->pFeatureMessages)
    {
        ...
        else if (pMessageDescriptor->pFeatureMessages->messageType == ChiFeature2MessageType::SubmitRequestNotification)
        {
            ...
            pFeature2Wrapper->m_pSubmissionLock->Lock();
            pFeature2Wrapper->m_pUsecaseBase->SubmitRequest(&submitRequest);
            pFeature2Wrapper->m_pSubmissionLock->Unlock();
        }
    }
}
    ...
    VOID*                       pPrivateCallbackData)
{
    ...
    if (NULL != pResult->pPartialResultMetadata)
    {
        ...
        if ((NULL != pPartialResultMetadata) &&
            (TRUE == m_pUsecaseBase->CheckIfPartialDataCanBeSent(sender, resultFrameIndex)))
        {
