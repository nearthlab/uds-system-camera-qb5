...
struct ChiFeature2GraphManagerCallbacks
{
    ...
    void(*ChiFeatureGraphProcessMessage)(
...
/// @brief Stuct for key to Stream Map used in graph manager
struct ChiFeature2StreamMapKey
{
    UINT32      featureId;      ///< featureId
    const CHAR* pTargetName;    ///< target buffer name
    UINT        instanceId;     ///< Instance Id of the feature

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// ChiFeature2StreamMapKey
    ///
    /// @brief  Constructor for ChiFeature2StreamMapKey
    ///
    /// @return None
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ChiFeature2StreamMapKey()
    ...
    // NOWHINE DC002a: Need comparison operator for convenient comparison
    BOOL operator==(const ChiFeature2StreamMapKey& rRHS) const
    {
        return ((featureId   == rRHS.featureId) &&
                (pTargetName == rRHS.pTargetName) &&
                (instanceId  == rRHS.instanceId));
    }

    // NOWHINE DC002a: Need comparison operator for convenient comparison
    BOOL operator<(const ChiFeature2StreamMapKey& rRHS) const
    {
        if (featureId != rRHS.featureId)
        {
            return featureId < rRHS.featureId;
        }
        else if (instanceId != rRHS.instanceId)
        {
            return instanceId < rRHS.instanceId;
        }
        else
        {
            return pTargetName < rRHS.pTargetName;
        }
    }
};

typedef std::map<const ChiFeature2StreamMapKey, ChiStream*> ChiFeature2StreamTargetMap;              ///< StreamTarget map
