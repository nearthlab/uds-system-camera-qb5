    ...
    std::vector<ChiFeature2Base*>& rpEnabledFeatures)
{
    ...
    if (NULL != pFeatureGraphDesc)
    {
        ...
        for (UINT j = 0; j < numFeatureInstances; j++)
        {
            ...
            feature2CreateInputInfo.pUsecaseStreamTargetMap       = pFeaturePoolInputData->pUsecaseStreamTargetMap;
            ...
            if (m_pEnabledFeaturesMap.find(feature2InstanceId) == m_pEnabledFeaturesMap.end())
            {
                ...
                feature2CreateInputInfo.bFrameworkRAWStream   = negotiationOutput.isFrameworkRawStream;
    ...
    BOOL isSynchronousFlush)
{
    ...
    if (FALSE == isSynchronousFlush)
    {
        ...
        if (m_numFeaturesFlushed != m_pFeatureList.size())
        {
            result = m_pFlushCondition->TimedWait(m_pFlushMutex->GetNativeHandle(), 1000);
        }

        if (CDKResultSuccess != result)
        {
            CHX_LOG_ERROR("Flush TIMEOUT!!");
            CdkUtils::RaiseSignalAbort();
        }
