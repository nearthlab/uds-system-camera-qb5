        ...
        do
        {
            ...
            switch (requestState)
            {
                ...
                case ChiFeature2RequestState::OutputErrorNotificationPending:
                    result = HandleOutputErrorNotificationPending(pRequestObject, requestId);
                    if (CDKResultSuccess != result)
                    {
                        CHX_LOG_ERROR("HandleOutputErrorNotificationPending failed with result: %d", result);
                    }
                    canExecute = FALSE;
                    break;
                ...
                case ChiFeature2RequestState::Complete:
                    OnProcessingDependenciesComplete(pRequestObject);
                    break;
    ...
    ChiFeature2CreateInputInfo* pCreateInputInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        m_pUsecaseStreamTargetMap = pCreateInputInfo->pUsecaseStreamTargetMap;
    ...
    if (CDKResultSuccess == result)
    {
        m_pMaxHalReqCount = GetMaxHalRequests(pCreateInputInfo->pFeatureDescriptor);
    ...
    ChiFeature2CreateInputInfo* pCreateInputInfo)
{
    ...
    for (UINT8 stageIndex = 0; ((stageIndex < pFeatureDesc->numStages) && (CDKResultSuccess == result)); ++stageIndex)
    {
        ...
        {
            const ChiFeature2SessionDescriptor&   rSessionDesc = pCreateInputInfo->pFeatureDescriptor->pSession[sessionIdx];
            const ChiFeature2SessionInfo&         rSessionInfo = pStageDescriptor->pDependencyConfigDescriptor[sessionIdx];

            std::vector<ChiFeaturePipelineData*>& rPipelineDataList = m_pSessionData[sessionIdx]->pPipelineData;
    ...
    const CHISTREAMCONFIGINFO* pStreams)
{
    ...
    else
    {
        for (UINT32 streamIdx = 0; streamIdx < pStreams->numStreams; streamIdx++)
        {
            ...
            m_streamMatched.push_back(FALSE);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2Base::SendSubmitRequestMessage(
    ChiPipelineRequest* pPipelineRequest
    ) const
{
    ...
    VOID*                       pPrivateCallbackData)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        else if (NULL != pCombinedCbData)
        {
            for (UINT8 cbIndex = 0; cbIndex < pCombinedCbData->numCallbackData; ++ cbIndex)
            {
                // FRO might not exist per frame data, we will let the graph take care of which message types need a valid
                // FRO and which messages don't
                ChiFeatureFrameCallbackData*    pFrameCbData   = pCombinedCbData->pCombinedCallbackData[cbIndex];
        ...
        {
            CHX_LOG_ERROR("Frame callback data is null for message type:%d; not propagating any notification",
    ...
    ) const
{
    ...
    do
    {
        pPortIdentifier = &identifierList.back();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2Base::ProcessFeatureMessage(
    ...
    ChiFeatureStreamData*             pOutputStreamData)
{
    ...
    {
        if ((NULL != m_pUsecaseStreamTargetMap) &&
            (ChiStreamTypeOutput == pOutputStreamData->pTargetStream->streamType))
        {
            ChiFeature2StreamMapKey key = {};
            key.featureId               = GetFeatureId();
            key.instanceId              = GetInstanceProps()->instanceId;
            key.pTargetName             = pTargetDesc->pTargetName;

            CHX_LOG_INFO("Add feature %u Target %s instance %d stream %p",
                key.featureId,
                key.pTargetName,
                key.instanceId,
                pOutputStreamData->pTargetStream);
            m_pUsecaseStreamTargetMap->insert({key, pOutputStreamData->pTargetStream});
        }
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ChiFeature2Base::GetMaxHalRequests
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT32 ChiFeature2Base::GetMaxHalRequests(
    const ChiFeature2Descriptor* pFeatureDesc)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        UINT32*                       phMaxHalReqHandle  = NULL;


        for (UINT32 sessionIdx = 0; sessionIdx < pFeatureDesc->numSessions; sessionIdx++)
        {
            const ChiFeature2SessionDescriptor* pSessionDesc = &pFeatureDesc->pSession[sessionIdx];
            if (NULL != pSessionDesc)
            {
                globalId.session = pSessionDesc->sessionId;
                pSessionData     = GetSessionData(&globalId);

                pPipelineData    = GetPipelineData(&pSessionData->globalId);
                if ((NULL != pPipelineData) && (NULL != pPipelineData->pPipeline))
                {
                    ...
                    result = vendorTagOps.pQueryVendorTagLocation("com.qti.chi.maxlivependingrequests",
                                        "maxLivePendingRequests", &vendorTagMetaOwner);
                    if (CDKResultSuccess == result)
                    {
                        phMaxHalReqHandle      = static_cast<UINT32*>(pMetadata->GetTag(vendorTagMetaOwner));
                        if ((NULL != phMaxHalReqHandle) && (*phMaxHalReqHandle > maxHalRequests))
                        {
                            maxHalRequests  = *phMaxHalReqHandle;
                        }
                    }
                }
            }
    ...
    CHX_LOG_INFO("maxHalRequests : %d", maxHalRequests);
    return maxHalRequests;
}
        ...
        case ChiFeature2RequestObjectOpsType::InputDependency:
        {
            switch(handleType)
            {
                case ChiFeature2HandleType::BufferMetaInfo:
                {
                    ...
                    {
                        pRequestObject->GetBufferInfo(opsType,
                            &pPortDesc->globalId, &pBufferMetaInfo->hBuffer, &pBufferMetaInfo->key,
                            requestId, 0);

                        pRequestObject->GetMetadataInfo(opsType,
                            &pPortDesc->globalId, &pBufferMetaInfo->hMetadata, requestId, 0);
                    }
                    break;
                }
                default:
    ...
    ) const
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        BOOL syncSequenceIdOnPorts = FALSE;

        GetZSLQueueForPort(pRequestObject, &pOutputBuffer->globalId, &pZSLQueueData);
        UINT32 minLatestSequenceId = 0;
        if (NULL != pZSLQueueData)
        {
            minLatestSequenceId = pZSLQueueData->frameNumbers.front();
            pZSLQueueData->frameNumbers.pop();
            CHX_LOG_INFO("Found ZSL Q for port  popping frame %d", minLatestSequenceId);
            syncSequenceIdOnPorts = TRUE;
        }
        ...
        for (UINT8 numPorts = 0; numPorts < numData; numPorts++)
        {
            ...
            if (CDKResultSuccess == result)
            {
                ...
                {
                    if (TRUE == syncSequenceIdOnPorts)
                    {
                        latestAvailableSequenceId = minLatestSequenceId;
                    }
                    else
                    {
                    ...
                    CHX_LOG_INFO("%s, [%u, %u, %u, %d, %d] : syncSequenceIdOnPorts = %d, latestAvailableSequenceId = %d",
                        pRequestObject->IdentifierString(),
                    ...
                    if (ChiFeature2PortType::ImageBuffer == pPortData->globalId.portType)
                    {
                        result = pBufferManager->GetTargetBuffers(bufferOffset, 1, &hBuffer[0], TRUE);
                    ...
                    {
                        result = pMetaManager->GetTargetBuffers(bufferOffset, 1, &hBuffer[0], TRUE);
                        pOutputBuffer[numPorts].pBufferMeta[numBuf].hBuffer = hBuffer[0];
                        pOutputBuffer[numPorts].pBufferMeta[numBuf].key     =
                            pPipelinedata->pPipeline->GetMetadataClientId();
                    }
    ...
    ChiFeature2RequestObject* pRequestObject)
{
    ...
    if (FALSE == pRequestObject->GetOutputPortsProcessingCompleteNotified())
    {
        if ((TRUE == pRequestObject->AllOutputPortsProcessingComplete()) &&
            ((pRequestObject->GetNumRequests() - 1) == pRequestObject->GetCurRequestId()))
        {
    ...
    ) const
{
    ...
    if (NULL != pResultMetadata)
    {
        ...
        BOOL doDebugDataCopy = IsMasterCamera(pResultMetadata, m_pCameraInfo);
        if (TRUE == doDebugDataCopy)
        {
            DebugDataCopy(pRequestObject, pInputMetadata);
        }
    }
    ...
    ) const
{
    ...
    {
        CHAR metaName[256] = { 0 };
        CdkUtils::SNPrintF(metaName, sizeof(metaName), "F2Base_Metadata_dump_%d", index);
    ...
    ) const
{
    ...
    if ((CDKResultSuccess == result) && (NULL != pStreamBuffer) && (ChiStreamFormatRaw10 == pStreamBuffer->pStream->format))
    {
        CHX_LOG_VERBOSE("Dump image format=%d, WxH=%dx%d, planeStride=%d, sliceHeight=%d",
                        pStreamBuffer->pStream->format,
                        pStreamBuffer->pStream->width,
                        pStreamBuffer->pStream->height,
        ...
        CdkUtils::SNPrintF(imageName, sizeof(imageName), "/data/vendor/camera/F2Base_dump_%dx%d_%d.raw",
            pStreamBuffer->pStream->width, pStreamBuffer->pStream->height, index);
