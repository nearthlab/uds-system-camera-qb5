...
public:
    ...
    static VOID getSensorDimension(
        ...
        UINT32 *sensorw, UINT32 *sensorh, UINT32 *pSensorbpp, UINT32 downscaleFactor,
    ...
    static ChiUsecase* DefaultMatchingUsecase(
        ...
        ChiUsecase**                    ppClonedUsecase); ///< Stream configuration
...
private:
    ...
    /// Is the stream config a match for video only
    static BOOL IsVideoNoPreviewStreamConfig(
        const camera3_stream_configuration_t* pStreamConfig);
    ...
    /// Is the stream config a match for Video + 2Previews
    static BOOL IsVideoWithTwoPreviewStreamConfig(
        const camera3_stream_configuration_t* pStreamConfig);
    ...
    static BOOL            HFRNoPreviewUsecase;            ///< Flag to select video no preview usecases
