    ...
    ChiCaptureResult* pResult)
{
    ...
    if (NULL != pResult->pOutputMetadata)
    {
        ...
        if (NULL != pChiOutputMetadata)
        {
            BOOL* pIs3HDRTriggered = static_cast<BOOL*>(pChiOutputMetadata->GetTag(
                                           "org.quic.camera2.statsconfigs", "AECIsInsensorHDR"));
            if (NULL != pIs3HDRTriggered)
            {
                CHX_LOG_INFO("RealtimeCaptureResult, internalFrameNum=%u, pIs3HDRTriggered=%u, before=%u",
                    internalFrameNum, *pIs3HDRTriggered, m_isInSensorHDR3ExpAECTrigger);
                m_isInSensorHDR3ExpAECTrigger = *pIs3HDRTriggered;
            }
        }
    }
    ...
    VOID*                       pPrivateCallbackData)
{
        ...
        (ChiMessageTypeTriggerRecovery == pMessageDescriptor->messageType))
    {
        ...
        pUsecase->m_sessionInfo[pSessionPrivateData->sessionId].m_bufferQueue[index].errFlag[pipelineIndex] = TRUE;
    ...
    camera3_stream_configuration_t* pStreamConfig)
{
    ...
    else if (NULL != m_pTargetSnapshotStream)
    {
        ...
        for (UINT i = 0; i < m_numOfPhysicalDevices; i++)
        {
            ...
            CHX_LOG_INFO("RDI resolution:%ux%u,camera index:%d pFullOptions: %p",
                         pObj->m_RAWWidth,
                         pObj->m_RAWHeight,
                         i,
                         pFullOptions);
...
CDKResult UsecaseMultiCamera::CreateBufferManagers()
{
    ...
    for (UINT32 i = 0; i < m_numOfPhysicalDevices; i++)
    {
        ...
        if (TRUE == m_isOfflineRequired)
        {
            ...
            bufCreateData.producerFlags = GRALLOC1_PRODUCER_USAGE_CAMERA;
            ...
            if (CDKResultSuccess == result)
            {
                for (UINT32 i = 0; i < requestInfo.numOfRequest; i++)
                {
                    ...
                    if (TRUE == m_isInSensorHDR3ExpCapture)
                    {
                        ...
                        {
                            CHX_LOG_INFO("Set m_seamlessInSensorState to InSensorHDR3ExpStop, internalFrameNum %d",
                                         internalFrameNum);
                            m_seamlessInSensorState = SeamlessInSensorState::InSensorHDR3ExpStop;
                            SendNopRequest(2, m_MCCResult);
                        }
                    }
    ...
    const CHICAPTURERESULT* pResult)
{
    ...
    if ((TRUE == hasPreviewStream) && (NULL != rtPreviewBuffer))
    {
        ...
        if (0 < pResult->numOutputBuffers)
        {
            if (TRUE == (pSessionInfo->m_bufferQueue[internalFrameNumIndex].errFlag[pipelineIndex]))
            {
    ...
    if ((NULL != pRDIMetadata) && (NULL != pChiOutputMetadata))
    {
        ...
        if (1 <= ExtensionModule::GetInstance()->EnableDumpDebugData())
        {
            BOOL isMaster = (m_requestMapInfo[internalFrameNumIndex].masterCameraID ==
                m_sessionInfo[REALTIME_SESSION].m_pSession->GetCameraId(rtIndex) ? TRUE : FALSE);
    ...
    const CHICAPTURERESULT* pResult)
{
    ...
    {
        CHAR* pData = NULL;
        ChxUtils::AndroidMetadata::GetVendorTagValue(pChiOutputMetadata, VendorTag::DebugDataTag, (VOID**)&pData);
        if (NULL != pData)
        {
            ...
            m_debugDataRealTime.queueIndex = (++m_debugDataRealTime.queueIndex) % numberOfSlots;
        }
        else
        {
    ...
    const BOOL isMaster)
{
    ...
    if (FALSE == isMaster)
    {
        ...
        result = ChxUtils::AndroidMetadata::SetVendorTagValue((VOID*)pRequest->pMetadata,
    ...
    else if (TRUE == ChxUtils::AndroidMetadata::IsVendorTagPresent(reinterpret_cast<const VOID*>(pRequest->pMetadata),
             VendorTag::DebugDataTag))
    {
        CHAR* pData = NULL;
        ChxUtils::AndroidMetadata::GetVendorTagValue((VOID*)(pRequest->pMetadata), VendorTag::DebugDataTag, (VOID**)&pData);
        DebugData* pDebug = reinterpret_cast<DebugData*>(pData);
    ...
    Feature* pFeature)
{
    ...
    if ((FeatureType::MFNR == pFeature->GetFeatureType()) && (TRUE == m_isInSensorHDR3ExpCapture))
    {
