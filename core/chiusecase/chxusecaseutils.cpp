...
BOOL UsecaseSelector::HFRNoPreviewUsecase   = FALSE;
    ...
    const camera3_stream_t* pStream)
{
    ...
    if ((NULL                    != pStream)                &&
        ...
        ((HAL_PIXEL_FORMAT_RAW10 == pStream->format) ||
         (HAL_PIXEL_FORMAT_RAW12 == pStream->format) ||
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL UsecaseSelector::IsVideoNoPreviewStreamConfig(
    const camera3_stream_configuration_t* pStreamConfig)
{
    ...
    {
        return IsVideoStream(pStreamConfig->streams[0]);
    }
    else
    {
        return false;
    ...
    const camera3_stream_configuration_t* pStreamConfig)
{
    ...
    if (pStreamConfig->num_streams >= 2)
    {
        bCntMatch = TRUE;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// UsecaseSelector::IsVideoWithTwoPreviewStreamConfig
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL UsecaseSelector::IsVideoWithTwoPreviewStreamConfig(
    const camera3_stream_configuration_t* pStreamConfig)
{
    BOOL bVideoPreviewStream = FALSE;
    BOOL bSnapshotStream     = FALSE;

    if (pStreamConfig->num_streams == 3)
    {
        for (UINT i = 0; i < pStreamConfig->num_streams; i++)
        {
            if (TRUE == IsYUVSnapshotStream(pStreamConfig->streams[i]) ||
                TRUE == IsJPEGSnapshotStream(pStreamConfig->streams[i]))
            {
                bSnapshotStream = TRUE;
            }
            else if (TRUE == IsVideoStream(pStreamConfig->streams[i]) ||
                     TRUE == IsPreviewStream(pStreamConfig->streams[i]))
            {
                bVideoPreviewStream = TRUE;
            }
        }
    }

    return ((TRUE == bVideoPreviewStream) && (FALSE == bSnapshotStream));
}
    ...
    camera3_stream_configuration_t* pStreamConfig)
{
    ...
    else
    {
        ...
        {
            case 1:
                if (TRUE == IsVideoNoPreviewStreamConfig(pStreamConfig))
                {
                    CHX_LOG_CONFIG("default usecase selected");
                    HFRNoPreviewUsecase = TRUE;
            ...
            case 2:
                ...
                /// VideoLiveShot can support only video + snapshot stream
                /// combination without preview stream.
                /// APP can just configure two streams(video&snapshot stream)
                /// for VideoLiveShot.
                if (TRUE == IsVideoLiveShotConfig(pStreamConfig))
                {
    ...
    ChiUsecase**                    ppClonedUsecase)
{
    ...
    isOfflineIFE                          = ExtensionModule::GetInstance()->IsOfflineIFE();
    ...
    auto UsecaseMatches = [&pStreamConfig, &pruneSettings, &pClonedUsecase](const ChiUsecase* const pUsecase) -> BOOL
    {
        BOOL isMatching = FALSE;
        /// clone usecase
        /// In multi-camera concurrency usecase, multiple cameras maybe
        /// select and configure the same one usecase target, which will
        /// cause a conflict since these usecase targets are defined as
        /// static struct variable. So clone usecase before matching to
        /// fix this conflict.
        UINT32 pipelineIDMap[MaxPipelines] = {0};
        UINT32 totalPipelineCount          = pUsecase->numPipelines;
        for (UINT32 i = 0; i < totalPipelineCount; i++)
        {
            pipelineIDMap[i] = i;
        }

        pClonedUsecase = UsecaseSelector::CloneUsecase(pUsecase, totalPipelineCount, pipelineIDMap);
        if (NULL != pClonedUsecase)
        {
            CHX_LOG_INFO("Clone usecase successfully for default usecase, pClonedUsecase:%p", pClonedUsecase);
        }
        else
        {
            CHX_LOG_ERROR("Failed to Clone usecase");
            return isMatching;
        }

        isMatching = IsMatchingUsecase(pStreamConfig, pClonedUsecase, &pruneSettings);
        if ((FALSE == isMatching) && (NULL != pClonedUsecase))
        {
            CHX_LOG_INFO("Destroy pClonedUsecase since usecase mismatch");
    ...
    if (pStreamConfig->num_streams <= ChiMaxNumTargets)
    {
            ...
            (StreamConfigModeNormal == pStreamConfig->operation_mode))
        {
            ...
            case 2:
                ...
                {
                    pSelectedUsecase = pClonedUsecase;
                }
                break;
            case 3:
            ...
            {
                CDKResult res = PruneUsecaseDescriptor(pClonedUsecase,
                ...
                && (StreamConfigModeConstrainedHighSpeed == pStreamConfig->operation_mode) && (NULL == pSelectedUsecase))
        {
            ...
            {
                PruneUsecaseDescriptor(pClonedUsecase, pruneSettings.numSettings,
                    pruneSettings.pVariants, &pSelectedUsecase);
        ...
        else if (NULL == pSelectedUsecase)
        {
            ...
            if ((TRUE == IsVideoWithTwoPreviewStreamConfig(pStreamConfig)) &&
                (NULL == pSelectedUsecase)                                 &&
                (TRUE == UsecaseMatches(&Usecases3Target[UsecaseVideoWithTwoPreviewsId])))
            {
                CHX_LOG("Selected VideoWithTwoPreviews usecase");
                pSelectedUsecase = pClonedUsecase;
            }
    ...
    for (/*Initialized outside*/; numTargetsExpected < ChiMaxNumTargets; numTargetsExpected++)
    {
        ...
        for (UINT i = 0; i < pChiTargetUsecases->numUsecases; i++)
        {
            ...
            if (pChiUsecaseDescriptor == g_pUsecaseVideoHFRNo3ANoPreview)
            {
               if (TRUE == HFRNoPreviewUsecase)
               {
                    CHX_LOG_VERBOSE("Will not select video HFR no 3A no preview Usecase: %s", pChiUsecaseDescriptor->pUsecaseName);
                    continue; // Prevent selecting video HFR no 3A no preview if above flags are disabled
               }
            }
            if (TRUE == isOfflineIFE)
            {
                if (pStreamConfig->num_streams == 2)
                {
                    AddSetting("Snapshot", "HEIC");
                }
                if ((TRUE == UsecaseMatches(g_pUsecaseOfflineIFEPreview)) && (NULL != pClonedUsecase))
                CDKResult res = PruneUsecaseDescriptor(pClonedUsecase,
                                                       pruneSettings.numSettings,
                                                       pruneSettings.pVariants,
                                                       &pSelectedUsecase);
            ...
            if ((TRUE == UsecaseMatches(pChiUsecaseDescriptor)) && (NULL != pClonedUsecase))
            {
                CDKResult res = PruneUsecaseDescriptor(pClonedUsecase,
                                                       pruneSettings.numSettings,
                                                       pruneSettings.pVariants,
        ...
        else if (NULL != pClonedUsecase) //go to match usecase in the next loop
        {
            CHX_LOG_INFO("Destroy pClonedUsecase %p since pSelectedUsecase is NULL", pClonedUsecase);
            UsecaseSelector::DestroyUsecase(pClonedUsecase);
            pClonedUsecase = NULL;
        }
    }
    ...
    if (NULL != pSelectedUsecase)
    {
        CHX_LOG_INFO("usecase %s, pipelineName %s, pClonedUsecase %p",
                     pSelectedUsecase->pUsecaseName,
                     pSelectedUsecase->pPipelineTargetCreateDesc->pPipelineName,
                     pClonedUsecase);
    }
    else
    {
    ...
    *ppClonedUsecase = pClonedUsecase;
        ...
        for (UINT targetInfoIdx = 0; targetInfoIdx < pUsecase->numTargets; targetInfoIdx++)
        {
            ...
            for (UINT streamId = 0; streamId < numStreams; streamId++)
            {
                ...
                if (NULL != pStream)
                {
                    ...
                    CHX_LOG("pStream %p, streamType = %d streamFormat = %d streamWidth = %d streamHeight = %d",
                            pStream, streamType, streamFormat, streamWidth, streamHeight);
                    ...
                    if (TRUE == isMatching)
                    {
                        if (pStream->format == ChiStreamFormatRaw12)
                        {
                            pStream->streamParams.bpp = 12;
                        }
                        else if (pStream->format == ChiStreamFormatRaw16)
                        {
                            pStream->streamParams.bpp = 16;
                        }
    ...
    GrallocUsage    grallocUsage)
{
    ...
    for (UINT i = 0; i < NumImplDefinedFormats; i++)
    {
        ...
        {
            if ((ChiFormatRawPlain16LSB10bit == AllowedImplDefinedFormats[i]) ||
                (ChiFormatRawPlain16LSB12bit == AllowedImplDefinedFormats[i]) ||
                (ChiFormatRawPlain16LSB14bit == AllowedImplDefinedFormats[i]) ||
    ...
    const ChiBufferFormat* pMatchingFormats)
{
    ...
    for (UINT32 i = 0; i < numFormats; i++)
    {
        if (((ChiStreamFormatRaw16 == streamFormat) && (DataspaceDepth != pStream->dataspace)) &&
                  ((ChiFormatRawPlain16LSB10bit == pMatchingFormats[i]) ||
                   (ChiFormatRawPlain16LSB12bit == pMatchingFormats[i]) ||
                   (ChiFormatRawPlain16LSB14bit == pMatchingFormats[i])))
        {
            isMatching = TRUE;
        }
        ...
        else if (((ChiStreamFormatRawOpaque == streamFormat) || (ChiStreamFormatRaw10 == streamFormat) || (ChiStreamFormatRaw12 == streamFormat)) &&
                 ((ChiFormatRawMIPI10 == pMatchingFormats[i]) ||
                  (ChiFormatRawMIPI12 == pMatchingFormats[i]) ||
                  (ChiFormatRawMIPI14 == pMatchingFormats[i])))
        {
            isMatching = TRUE;
        }
...
VOID UsecaseSelector::DestroyUsecase(ChiUsecase *pUsecase)
{
    ...
    if (NULL != pUsecase)
    {
        CHX_LOG_INFO("DestroyUsecase: %p", pUsecase);
...
VOID ImageBuffer::Destroy()
{
    ...
    if (NULL != pGrallocBuffer)
    {
        ...
        {
            bo_handle = ((UINT64)(pHandle->data[1]) & 0xFFFFFFFF);
            bo_handle = (bo_handle << 32) | ((UINT64)(pHandle->data[2]) & 0xFFFFFFFF);
...
#ifdef LE_CAMERA
const std::unordered_map<INT, INT> GBMBufferAllocator::GBMUsageMap =
{
    ...
    //GRALLOC_USAGE_HW_CAMERA_READ  == GRALLOC1_CONSUMER_USAGE_CAMERA
    {GRALLOC_USAGE_HW_CAMERA_READ,   GBM_BO_USAGE_CAMERA_READ_QTI},
    //GRALLOC_USAGE_HW_CAMERA_WRITE == GRALLOC1_PRODUCER_USAGE_CAMERA
...
UINT GBMBufferAllocator::GetGbmUsageFlag(UINT32 gbm_format, UINT32 cons_usage, UINT32 prod_usage)
{
    UINT gbm_usage  = 0;
    switch(gbm_format)
    {
...
buffer_handle_t GBMBufferAllocator::AllocateNativeHandle(struct gbm_bo* bo)
{
    ...
    if(bo != NULL)
    {
        bo_handle = reinterpret_cast<UINT64>(bo);
