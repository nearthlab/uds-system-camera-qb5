...
VOID UsecaseSuperSlowMotionFRC::HandleCopyInterpolatedOutput_l()
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        UINT32 diff             = GetTvDiff(&end, &start);
        UINT64 tsDelta          = pRequest->timestamp - m_lastCopiedTimestamp;
        m_lastCopiedTimestamp   = pRequest->timestamp;
        CHX_LOG("copying frc output from frame_number=%u to frame_number=%u, ts=%" PRId64 ", delta=%" PRId64 ", duration=%u",
    ...
    camera3_stream_configuration_t* pStreamConfig)  ///< Stream configuration
{
...

    {
        ...
        m_pChiUsecase       = UsecaseSelector::DefaultMatchingUsecase(pStreamConfig, &m_pClonedUsecase);
        UINT* cameraIds     = NULL;
        UINT  pipelineNum;
    ...
    VOID*                       pPrivateCallbackData)
{
    ...
    if (ChiMessageTypeShutter == pMessageDescriptor->messageType)
    {
        ...
        if (TRUE == pCr->isPreviewRequest)
        {
            CHX_LOG("frame_number=%u: (PREVIEW) issuing shutter message, timestamp=%" PRId64,
                    pCr->frameNumber, pCr->tsNotify);

            m_pAllResultsMutex->Lock();
            if ((m_nextAppMessageFrame == (frameNum - 1)) &&
                (TRUE == GetSSMCaptureRequest(frameNum - 1)->issueVideoError))
            {
                m_nextAppMessageFrame++;
                CHX_LOG("moving m_nextAppMessageFrame ahead, now at: %" PRIu64,
                        m_nextAppMessageFrame);
            }
