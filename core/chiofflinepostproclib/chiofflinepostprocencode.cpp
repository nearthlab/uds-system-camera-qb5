...
CDKResult ChiOfflinePostprocEncode::Setup()
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CDKResultSuccess != result)
        {
            CHX_LOG_ERROR("SetupCamera failed!");
        }
    }
    else
    {
        CHX_LOG_ERROR("LoadChiOps failed!");
    }
    ...
    ChiFeature2Messages*        pMessages)
{
    ...
    {
        CHX_LOG_ERROR("Invalid argument: pFeatureRequestObj is NULL");
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (NULL != pFeature2Inf)
        {
            ...
            if ((NULL != pMessages->pFeatureMessages) &&
                (ChiFeature2MessageType::SubmitRequestNotification == pMessages->pFeatureMessages->messageType))
            {
                CHX_LOG_VERBOSE("ChiOfflineEncode: SubmitRequest API called");
                ExtensionModule::GetInstance()->SubmitRequest(const_cast<CHIPIPELINEREQUEST*>
                    (&(pMessages->pFeatureMessages->message.submitRequest)));
                CHX_LOG_VERBOSE("ChiOfflineEncode: SubmitRequest API completed");
            }
    ...
    if (NULL != m_pFeature2Base)
    {
        ...
        if (NULL != m_pFeature2RequestObject)
        {
            ...
            do
            {
                ...
                CHX_LOG_INFO("Waiting for output notifiation");
                m_pFeature2RequestStateComplete->TimedWait(m_pFeature2RequestStateMutex->GetNativeHandle(), 100);
            ...
            LockFeature2Object();
            CHX_LOG_INFO("Destroy feature2 object");
            m_pFeature2RequestObject->Destroy();
            m_pFeature2RequestObject = NULL;
...
{
    CHX_LOG_INFO("Output notifiation is received");
