...
PostProcService::PostProcService()
{
    ...
    if (NULL != pLib)
    {
        ...
        {
            ALOGE("%s: postproc function pointers are NULL, dlsym failed, %p, %p, %p",
    ...
    const sp<IPostProcServiceCallBacks>&    rCallback)
{
    ...
    if (NULL != pSession)
    {
        ...
        m_pPostProcInstance.insert(pSession);
    ...
    uint64_t cookie)
{
    ...
    if (m_pPostProcInstance.find(pSession) != m_pPostProcInstance.end())
    {
