...
static LogicalCameraConfiguration logicalCameraConfiguration[] =
{
    ...
    {13,       LogicalCameraType_Default, TRUE,      1,    {{7,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},   7   }, /// Sensor7
};
...
#ifdef LE_CAMERA
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ExtensionModule::SetCameraPerfMode
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ExtensionModule::SetCameraPerfMode()
{
    FILE* pFile = NULL;
    UINT CamPid = getpid();

    const CHAR* pSchedtuneName    = "/sys/fs/cgroup/schedtune/camera/schedtune.colocate";
    const CHAR* pCgroupName       = "/sys/fs/cgroup/schedtune/camera/cgroup.procs";
    const CHAR* pSchedtuneDirName = "/sys/fs/cgroup/schedtune/camera";

    CHX_LOG("current CamPid:%u", CamPid);

    // Create camera directory in schedtune
    CdkUtils::Mkdir(pSchedtuneDirName, 0777);

    // write 1 to schedtune.colocate
    std::string EnableString = std::to_string(1);
    pFile = CdkUtils::FOpen(pSchedtuneName, "wb");
    if (NULL != pFile)
    {
        CHX_LOG("write value:%s into %s", EnableString.c_str(), pSchedtuneName);
        CdkUtils::FWrite(EnableString.c_str(), EnableString.size(), 1, pFile);
        CdkUtils::FClose(pFile);
        pFile = NULL;
    }
    else
    {
        CHX_LOG_ERROR("Failed to fopen file:%s", pSchedtuneName);
        return;
    }

    // write camera PID to cgroup.procs
    // cgroup is linux control groups which allow processes
    // to be added in hierarchical groups whose usage of
    // resources(memory, cpu, I/O and so on) will be limited
    // and monitored.
    std::string PidString = std::to_string(CamPid);
    pFile = CdkUtils::FOpen(pCgroupName, "wb");
    if (NULL != pFile)
    {
        CHX_LOG("write value:%s into %s", PidString.c_str(), pCgroupName);
        CdkUtils::FWrite(PidString.c_str(), PidString.size(), 1, pFile);
        CdkUtils::FClose(pFile);
        pFile = NULL;
    }
    else
    {
        CHX_LOG_ERROR("Failed to fopen file:%s", pCgroupName);
        return;
    }
}
#endif
...
ExtensionModule::~ExtensionModule()
{
    ...
    for (UINT type = 0; type < OFFLINELOG_MAX_TYPE; type++)
    {
        SignalOfflineLoggerThread((OfflineLoggerType)type);

        ChxUtils::ThreadTerminate(m_pOfflineLoggerThread[type].hThreadHandle);
    ...
    VOID*     pPriv)
{
    ...
    BOOL  isCameraResourceCostEnable      = TRUE;

    isCameraResourceCostEnable = ExtensionModule::GetInstance()->IsCameraResourceCost();
    ...
    if (CDKResultSuccess == result)
    {
        ...
        // Required for PIP camera
        if ((isCameraResourceCostEnable == TRUE) &&
            (m_logicalCameraInfo[logicalCameraId].maxResourceCost + activeResourceCost) > m_totalResourceBudget)
        {
            CHX_LOG_ERROR("ExtendOpen failed! for cameraId %d HW resource insufficient! "
                "maxResourceCost = %d, activeResourceCost = %d, m_totalResourceBudget = %d", logicalCameraId,
                m_logicalCameraInfo[logicalCameraId].maxResourceCost, activeResourceCost, m_totalResourceBudget);
        ...
        if (NULL != pLogicalCameraInfo->ppDeviceInfo)
        {
            ...
            if (TRUE == isValid)
            {
                ...
                if (LogicalCameraType_Default != pLogicalCameraInfo->logicalCameraType)
                {
                    ...
                    UpdateStaticSettings(m_numLogicalCameras);
    ...
    VOID**                          pPrivate)
{
    ...
    *pIsOverrideEnabled   = FALSE;
    ...
    //Get force flag from pStreamConfig->operation_mode bit24
    ...
    if ((isVideoMode == TRUE) && (operation_mode != 0))
    {
        ...
        else
        {
            ...
            fps_matched = TRUE;
            CHX_LOG_INFO("operation_mode: %d, numSensorModes: %d, fps %d", operation_mode, numSensorModes,fps);
        }
    }

    // Use the enforce resolution index, work same as camxsettings.xml::overrideForceSensorMode
    if ((CDKResultSuccess == result) && (isSensorModeForce != 0) && (operation_mode != 0))
    {
        *m_pForceSensorMode = operation_mode - 1;
        CHX_LOG_INFO("normal operation_mode:0x%x, force sensor_mode:%d",
                      pStreamConfig->operation_mode, *m_pForceSensorMode);
    }
    ...
    if (CDKResultSuccess == result)
    {
            ...
            (StreamConfigModeSuperSlowMotionFRC == pStreamConfig->operation_mode))
        {
                ...
                (30 >= maxSessionFps))
            {
                ...
                if(fps_matched == TRUE){
                    minSessionFps = maxSessionFps = fps;
                    m_usecaseMaxFPS = maxSessionFps;
                }
    ...
    UINT32 logicalCameraId)
{
    ...
    CHX_LOG_INFO("CHI override has successfully recovered CameraId %d. use case is created for next request = %d",
        logicalCameraId, m_firstFrameAfterRecovery[logicalCameraId]);
    ...
    for (UINT i = 0; i < numTokens; i++)
    {
        switch (pTokens[i].id)
        {
            ...
            case static_cast<UINT32>(ChxSettingsToken::MaxHALRequests) :
                ...
                break;
            case static_cast<UINT32>(ChxSettingsToken::OfflineIFEEnable) :
                m_pOfflineIFEEnable               = &m_pConfigSettings[i];
                break;
            case static_cast<UINT32>(ChxSettingsToken::CameraResourceCostEnable) :
                m_pCameraResourceCostEnable       = &m_pConfigSettings[i];
                break;
            default:
                break;
        }
