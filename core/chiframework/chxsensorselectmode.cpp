    ...
    const DesiredSensorMode* pDesiredSensorMode)
{
    ...
    // Force isFound TRUE during desired caps <= 1 for QuadCFA sensor fullsize usage.
    if ((1 >= pDesiredSensorMode->sensorModeCaps.value && 1 == pModeInfo->sensorModeCaps.u.QuadCFA))
    {
        isFound = TRUE;
    }
    // Default select normal sensor modes if not configure mode caps
    else if (0 == pDesiredSensorMode->sensorModeCaps.value)
    {
