    ...
    if (CDKResultSuccess == result)
    {
        for (UINT32 i = 0; i < pMultiCamResultMetadata->numResults; i++)
        {
            ...
            if (NULL != cameraSettings[i].pMetadata)
            {
                ...
                {
                    CHX_LOG_ERROR("Metadata tag MultiCameraId/ MetadataOwner/ isMaster is NULL");
                    result = CDKResultEFailed;
                }
            }
        }
    ...
    ChiMetadata* pMetadata)
{
    ...
    {
        camIdx = GetCameraIndex(pMultiCameraId->currentCameraId);
        CHX_ASSERT(INVALID_INDEX != camIdx);
        cameraId = m_camInfo[camIdx].cameraId;
    }
    else
    {
        CHX_LOG_ERROR("Metadata tag BokehInputMeta is NULL");
        result = CDKResultEFailed;
    }
