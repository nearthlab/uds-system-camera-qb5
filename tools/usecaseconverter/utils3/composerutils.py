...
class SegmentInfo:
    ...
    def merge_info_into_pipeline(self, input_filename, output_filename):
        ...
        with open(output_filename, 'w+') as fd:
            ...
            fd.write(ET.tostring(root).decode('utf-8'))

    def __str__(self):
        result = "SegmentInfo: (Number of Segments: %d | Segments: [%s])"
        segment_str = ", ".join([str(key) for key in list(self.segments.keys())])
        result = result % (len(list(self.segments.keys())), segment_str)
        return result
    ...
    for child_element in list(search_root):
        if child_element.tag == element_type:
            ...
            if match_id not in unique_elements:
                ...
                unique_elements[match_id] = ElementInfo(search_root, child_element, list(elements_to_combine.keys()))
            else:
                logging.debug("Found exisiting UID: %s", match_id)
                existing = unique_elements[match_id]
                for combine_element_id, new_element in list(elements_to_combine.items()):
