...
class EasyMake(object):
    ...
    def _runCmd(self):
        ...
        if numTask == 0:
            ...
            for idx, rule in enumerate(self._commandQueue):
                print("( {}/{} ) Build {} ... ".format(idx+1, numTask, rule['msg']), end=' ')
    ...
    def clean(self):
        for rule in self._commandInput:
            print("Clean {} ... ".format(rule['msg']), end=' ')
            for t in rule['tgt']:
                if self._Verbose:
...
class CBuildCodeAndBinary(object):
    ...
    def __init__(self, chicdkroot, args):
        ...
        if self._ChiCdkRoot is None or not isinstance(self._ChiCdkRoot, str):
    ...
    def genCode(self):
        ...
        buildMK = EasyMake(force           = self._Force,
                           ...
                           debug           = self._Debug)
        for k,v in list(self.mk_conf.items()):
        ...
        buildCode = EasyMake(force           = self._Force, 
                             ...
                             verbose         = self._Verbose,
                             debug           = self._Debug)
        for k,v in list(self.code_conf.items()):
    ...
    def genBinaries(self):
        ...
        for binType, bins in list(self.bin_conf.items()):
            for items in bins:
                for name, files in list(items.items()):
