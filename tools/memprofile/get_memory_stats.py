...
RegisterPattern = r'^.*\[ INFO\]\[MEMMGR \] camxmempoolgroup\.cpp:\d+ .*RegisterBufferManager\(\) \[(.*)\]-->\[(.*)\] : NumBufMgrs=(.*), type=(.*), heap=(.*), Original Immediate=(.*), Max=(.*), SelfTuned Immediate=(.*), Max=(.*), needDedicatedBuffers=(.*), lateBinding=(.*), numBatched=(.*), deviceCount=(.*), devices=(.*), (.*), flags=(.*) width=(.*), height=(.*), format=(.*), size=(.*)'
