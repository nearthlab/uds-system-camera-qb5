...
*     CDKResult
**************************************************************************************************************************/
CDKResult ChiMetadataUtil::PatchingMetabufferPool(const CHAR* pFileName)
{
    std::ostringstream completeFilePath;
    ...
    if (pFp)
    {
        ...
        if (0 < fileLen)
        {
            ...
            if (NULL != pData)
            {
                ...
                {
                    CHX_LOG_ERROR("Zero bytes read for %s", fileNameString);
                }
            }
            else
            {
                CHX_LOG_ERROR("Fail to alloc %d bytes", fileLen);
    ...
    {
        CHX_LOG_ERROR("Cannot open file %s", fileNameString);
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            result |= m_pInputMetabufferPool[i]->SetTag(tagId, &pData, sizeof(VOID*));
        }
    }
