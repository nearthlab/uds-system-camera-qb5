...
CDKResult Feature2TestCase::PatchingMetadata(const CHAR* pFileName)
{
    CDKResult result = CDKResultSuccess;
    ...
    if (NULL != pInstance)
    {
        result = pInstance->PatchingMetabufferPool(pFileName);
    }
    else
    {
...
CDKResult Feature2TestCase::PatchingTuningMode()
{
    ...
    if (NULL != pInstance)
    {
        ...
        UINT controlModeValue = ANDROID_CONTROL_MODE_USE_SCENE_MODE;
...
CDKResult Feature2TestCase::PatchingStreamConfig(CHISTREAMCONFIGINFO* pStreamConfigInfo)
{
    ...
    {
        CF2_LOG_ERROR("Invalid stream config info!");
