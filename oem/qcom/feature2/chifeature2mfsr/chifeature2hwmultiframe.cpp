        ...
        if (CDKResultSuccess == result)
        {
            ...
            switch (stageId)
            {
                ...
                case HWMFStagePrefilter:
                    ...
                    ProcessHWMFPrefilterStage(pRequestObject, stageId, stageSequenceId, nextStageId);
                    break;

                case HWMFStageBlendInit:
                    ATRACE_ASYNC_BEGIN("ChiFeature2HWmultiframe::ProcessHWMFBlendLoopStage", HWMFStageBlendLoop);
                    ProcessHWMFBlendInitStage(pRequestObject, stageId, stageSequenceId, nextStageId);
                    break;

                case HWMFStageBlendLoop:
                    ProcessHWMFBlendLoopStage(pRequestObject, stageId, stageSequenceId, nextStageId);
                    break;

                case HWMFStagePostfilter:
                    ATRACE_ASYNC_BEGIN("ChiFeature2HWmultiframe::ProcessHWMFPostfilterStage", HWMFStagePostfilter);
                    ProcessHWMFPostfilterStage(pRequestObject, stageId, stageSequenceId, nextStageId);
                    break;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2HWmultiframe::ProcessHWMFPrefilterStage(
    ChiFeature2RequestObject * pRequestObject,
    UINT8                      stageId,
    ...
    if (NULL != pStageDescriptor)
    {
        ...
        if (NULL != pHWMFFeatureInfo)
        {
            ...
            for (UINT i = 0; i < pHWMFFeatureInfo->hwmfTotalNumFrames; i++)
            {
                ...
                {
                    DumpInputMetaBuffer(&inputMetaInfo, i);
                    DumpInputImageBuffer(&inputBufferInfo, i);
                }
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2HWmultiframe::ProcessHWMFBlendInitStage(
    ChiFeature2RequestObject * pRequestObject,
    UINT8                      stageId,
    UINT8                      stageSequenceId,
    UINT8                      nextStageId
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2HWmultiframe::ProcessHWMFBlendLoopStage(
    ChiFeature2RequestObject * pRequestObject,
    UINT8                      stageId,
    UINT8                      stageSequenceId,
    UINT8                      nextStageId
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2HWmultiframe::ProcessHWMFPostfilterStage(
    ChiFeature2RequestObject * pRequestObject,
    UINT8                      stageId,
    UINT8                      stageSequenceId,
    UINT8                      nextStageId
    ...
    if (NULL != pRequestObject)
    {
        ...
        if (pHWMFFeatureInfo != NULL)
        {
            ...
            for (UINT8 portIndex = 0; portIndex < pAllInputPorts->numPorts; portIndex++)
            {
                ...
                if (portId != blendInDS64PortIdentifier)
                {
                    selectedList.push_back(portId);
                }
            }
    ...
    ) const
{
    ...
    {
        CHAR metaName[256] = { 0 };
        CdkUtils::SNPrintF(metaName, sizeof(metaName), "HWMF_Metadata_dump_%d", index);
        DumpMetadata(pMetadata, metaName);
    }

    if ((CDKResultSuccess == result) && (NULL != pMetadata))
    {
        CHAR tuningModeFileName[256] = { 0 };
        CdkUtils::SNPrintF(tuningModeFileName, sizeof(tuningModeFileName), "/data/vendor/camera/HWMF_TuningMode_dump.txt");
        DumpTuningMode(pMetadata, tuningModeFileName);
    }
    ...
    ) const
{
    ...
    if ((CDKResultSuccess == result) && (NULL != pStreamBuffer))
    {
        ...
        CdkUtils::SNPrintF(imageName, sizeof(imageName), "/data/vendor/camera/HWMF_dump_%dx%d_%d.raw",
        ...
        if (NULL != pHWMFOutputStream)
        {
            ...
            else
            {
                ...
                if (snapshotWidth * snapshotHeight == 0)
                {
                    ...
                    {
                        pHWMFOutputStream->width  = pYUVSnapshotStream->width;
                        pHWMFOutputStream->height = pYUVSnapshotStream->height;
                    }
                }
            }
