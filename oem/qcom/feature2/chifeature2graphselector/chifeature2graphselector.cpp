...
VOID ChiFeature2GraphSelector::PopulateAllTables()
{
    ...
    keys =
    {
        ...
        SINGLE_CAMERA, { ControlCaptureIntentStillCapture ,
                         ...
                         ControlCaptureIntentManual},               {      ControlSceneModeDisabled,
                                                                           ...
                                                                           ControlSceneModeFacePriorityLowLight, },    { NoiseReductionModeOff,
                                                                                                                         NoiseReductionModeFast,
                                                                                                                         NoiseReductionModeMinimal,
                                                                                                                         NoiseReductionModeZeroShutterLag,},   { 0,
    ...
    // Inserting feature graph names to be used for stream mapping in graph manager.
    // DO NOT ADD feature graphs here
    m_featureGraphsForStreamMapping.insert({ RealTimeFeatureGraphDescriptor.pFeatureGraphName, TRUE });
    m_featureGraphsForStreamMapping.insert({ RTBayer2YUVJPEGFeatureGraphDescriptor.pFeatureGraphName, TRUE });
    m_featureGraphsForStreamMapping.insert({ RTBayer2YUVFeatureGraphDescriptor.pFeatureGraphName, TRUE });
    m_featureGraphsForStreamMapping.insert({ RTMFSRYUVFeatureGraphDescriptor.pFeatureGraphName, TRUE });
...
VOID ChiFeature2GraphSelector::Destroy()
{
    ...
    m_featureGraphsForStreamMapping.clear();
                            ...
                            else
                            {
                                if (SINGLE_CAMERA == it.cameraId)
                                {
                                    if ((ControlCaptureIntentZeroShutterLag == pKeysForRequest->captureIntent) ||
                                        ...
                                        (ControlCaptureIntentPreview == pKeysForRequest->captureIntent) ||
                                        (ControlCaptureIntentManual == pKeysForRequest->captureIntent))
                                    {
                                        ...
                                        // Disable MFNR burst
                                        else if ((CustomVendorTagValues::CustomVendorTagMFNR != pKeysForRequest->customVendorTag)
                                                 && (ControlCaptureIntentStillCapture == pKeysForRequest->captureIntent)
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (0 != CdkUtils::StrLen(pruneIDString))
        {
            ...
            if (prunedGraphDescIt != m_prunedFeatureGraphDescriptorsMap.end())
            {
                ...
                CHX_LOG_INFO("Feature descriptor found in prune map %p, %s", pFinalGraphDesc, pruneIDString);
            }
            else
            {
                ...
                else
                {
                    ...
                    CHX_LOG_INFO("InsertPruning graph:%s, %p", pPruneIDString, pFinalGraphDesc);
    ...
    ChiFeature2Hint*                 pCustomHints)
{
    ...
    if (NULL != pOutputMeta)
    {
        ...
        if (0 == status)
        {
            INT controlFLASHMode = static_cast<INT32>(*(entry.data.i32));
            CHX_LOG_INFO("controlFLASHMode value %d", controlFLASHMode);
            if (ANDROID_CONTROL_AE_MODE_ON_ALWAYS_FLASH == controlFLASHMode)
            {
    ...
    GraphDescriptorTables*           pGraphDescriptorTables)
{
    ...
    if (ANDROID_CONTROL_CAPTURE_INTENT_STILL_CAPTURE == keysForRequest.captureIntent)
    {
        ...
        keysForRequest.customVendorTag = GetCustomVendorTag(pAppSettings);
    }
...

{
    ...
    else
    {
        ...
        if (0 == status)
        {
            ...
            if (1 == burstShotFps)
            {
                customTagValue = CustomVendorTagValues::CustomVendorTagBurst;
            }
    ...
    CHICAPTUREREQUEST* pChiRequest)
{
    ...
    if (0 == status)
    {
        ...
        if (CaptureIntentAll.find(captureIntentValue) != CaptureIntentAll.end())
        {
            ...
            else if (captureIntent == ControlCaptureIntentVideoRecord)
            {
                for (UINT i = 0; i < pChiRequest->numOutputs; i++)
                {
                    if (pChiRequest->pOutputBuffers[i].pStream->format == ChiStreamFormatBlob)
                    {
                        ...
                        CHX_LOG_INFO("overriding capture intent: previous %d with %d ",
                            ControlCaptureIntentVideoRecord, captureIntent);
                        break;
                    }
                }
            }
