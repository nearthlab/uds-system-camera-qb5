    ...
    VOID* pCallbackData)
{
    ...
    if (NULL != pRealtimeCb)
    {
        ...
        if (NULL != pRealtimeInstance)
        {
            ...
            pRealtimeInstance->SendSubmitRequestMessage(pRealtimeCb->pRequest);
            ...
            case STAGE_REALTIME:
                if (FALSE == IsSWRemosaicSnapshot())
                {
                    if (0 == stageSequenceId)
                    {
                        ...
                        if ((TRUE == isRawEnabled) && (FALSE == isRawCbEnabled))
                        {
                            result = ProcessZSLRequest(pRequestObject);
                        }
                    }
    ...
    ) const
{
    ...
    for (UINT8 portIndex = 0; portIndex < pAllOutputPorts->numPorts; ++portIndex)
    {
        ...
        if (TRUE == dummyRequest)
        {
            ...
            else if (RealtimeOutputPortMetaData == portId)
            {
                selectedList.push_back(portId);
            }
        ...
        {
            if (RealtimeOutputPortRawCb == portId)
            {
                continue;
            }
            ...
            // Always add RT metadata port
            else if (RealtimeOutputPortMetaData == portId)
            {
                selectedList.push_back(RealtimeOutputPortMetaData);
            }
            // Always add FD port
            else if (RealtimeOutputPortFd == portId)
            {
    ...
    ) const
{
    ...
    if (previewRequest || videoRequest)
    {
    ...
    ) const
{
    ...
    CDKResult                      result             = CDKResultSuccess;
    ChiFeature2BufferMetadataInfo* pRealTimeRAWPortBufferInfo  = NULL;
    ChiFeature2BufferMetadataInfo* pRealTimeMetaDataBufferInfo = NULL;
    ChiFeature2BufferMetadataInfo* pRealTimeFdBufferInfo = NULL;

    if (FALSE == IsManualCaptureRequired(pRequestObject))
    {
        std::vector<ChiFeature2Identifier> selectedList;

        pRequestObject->GetFinalBufferMetadataInfo(RealtimeOutputPortRaw, &pRealTimeRAWPortBufferInfo,
            pRequestObject->GetCurRequestId());

        if (NULL != pRealTimeRAWPortBufferInfo)
        {
            ChiFeature2PortDescriptor    bufferPortDesc       = {RawZSLInputPortId, NULL, NULL};

            result = GetFeatureRequestOpsData(pRequestObject, ChiFeature2RequestObjectOpsType::InputDependency,
                    &bufferPortDesc, ChiFeature2HandleType::BufferMetaInfo, pRealTimeRAWPortBufferInfo);

            selectedList.push_back(RealtimeOutputPortRaw);

            pRequestObject->SetOutputGeneratedForPort(RealtimeOutputPortRaw, pRequestObject->GetCurRequestId());
        }
        ...
        {
            ChiFeature2PortDescriptor    metadataPortDesc     = {RawZSLInputMetaDataPortId, NULL, NULL};

            result = GetFeatureRequestOpsData(pRequestObject, ChiFeature2RequestObjectOpsType::InputDependency,
                &metadataPortDesc, ChiFeature2HandleType::BufferMetaInfo, pRealTimeMetaDataBufferInfo);

            if (CDKResultSuccess == result)
            {
                if (NULL != pRealTimeMetaDataBufferInfo->hBuffer)
                {
                    ChiMetadata*     pZSLMetadata     = NULL;
                    ChiMetadata*     pAppMetadata     = NULL;
                    CHIMETAHANDLE    hMetadataBuffer  = NULL;

                    result = GetMetadataBuffer(pRealTimeMetaDataBufferInfo->hBuffer,
            ...
            selectedList.push_back(RealtimeOutputPortMetaData);
            pRequestObject->SetOutputGeneratedForPort(RealtimeOutputPortMetaData, pRequestObject->GetCurRequestId());
        }

        pRequestObject->GetFinalBufferMetadataInfo(RealtimeOutputPortFd, &pRealTimeFdBufferInfo,
            pRequestObject->GetCurRequestId());
        if (NULL != pRealTimeFdBufferInfo)
        {
            ChiFeature2PortDescriptor    fdPortDesc           = {RawZSLInputFdPortId, NULL, NULL};

            result = GetFeatureRequestOpsData(pRequestObject, ChiFeature2RequestObjectOpsType::InputDependency,
                &fdPortDesc, ChiFeature2HandleType::BufferMetaInfo, pRealTimeFdBufferInfo);

            selectedList.push_back(RealtimeOutputPortFd);

            pRequestObject->SetOutputGeneratedForPort(RealtimeOutputPortFd, pRequestObject->GetCurRequestId());
        }
        ...
        if (CDKResultSuccess == result)
        {
            ...
            pResult                       = &featureMessage.message.result;
            pResult->resultIndex          = pRequestObject->GetCurRequestId();
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (NULL != pPipelineData)
        {
            ...
            if (NULL != pPipeline)
            {
                ...
                vendorTagOps.pQueryVendorTagLocation("com.qti.chi.maxlivependingrequests",
                                        "maxLivePendingRequests", &vendorTagMetaOwner);
                m_pMaxHalRequests      = static_cast<UINT32*>(pMetadata->GetTag(vendorTagMetaOwner));
                maxHalRequests         = *m_pMaxHalRequests - 1;

                CHX_LOG("maxHalRequests = %d", maxHalRequests);

                // In case with multiple sessions, we want m_pHALRequestSem to be initialized
                // with max from all sessions, so if the new session requires bigger semaphore.
                // count - recreate it.
                if ((NULL != m_pHALRequestSem) &&
                            (maxHalRequests > m_pHALRequestSem->GetSemaphoreCount()))
                {
                    m_pHALRequestSem->Destroy();
                    m_pHALRequestSem = NULL;
                }
                ...
                if (NULL == m_pHALRequestSem)
                {
                    m_pHALRequestSem = Semaphore::Create(maxHalRequests);
                }
            }
        }
    }
    ...
    StreamNegotiationOutput*        pNegotiationOutput)
{
    ...
    else
    {
        ...
        if (FALSE == pNegotiationInfo->pFeatureInstanceId->flags.isNZSLSnapshot)
        {
            ...
            pRDIStream = CloneStream(&RDIStream);
            ...
            if (NULL != pRDIStream)
            {
                GetSensorOutputDimension(physicalCamId,
                                         ...
                                         &pRDIStream->width,
                                         &pRDIStream->height,
                                         &pRDIStream->streamParams.bpp);
                if (pRDIStream->streamParams.bpp == 12)
                {
                    pRDIStream->format           = ChiStreamFormatRaw12;
                    pRDIStream->streamParams.bpp = 12;
                }
                ...
                for (UINT streamIdx = 0; streamIdx < numStreams; streamIdx++)
                {
                    ...
                    if (ppChiStreams[streamIdx]->format != ChiStreamFormatBlob)
                    {
                        if (ChiStreamFormatRaw10 == ppChiStreams[streamIdx]->format ||
                            ChiStreamFormatRaw12 == ppChiStreams[streamIdx]->format ||
                            ChiStreamFormatRaw16 == ppChiStreams[streamIdx]->format)
                        {
                            ...
                            {
                                CHX_LOG_ERROR("Framwork RAW stream is not selected:"
                                              "feature flags:%x, selected sensor rdi:%dx%d, current stream:%dx%d",
                            ...
                            else
                            {
                                ...
                                pNegotiationOutput->isFrameworkRawStream = TRUE;
                            }
                        }
                        else
                        {
