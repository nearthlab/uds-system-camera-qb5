    ...
    ChiFeature2Identifier* pKey)
{
         ...
         (0 == CdkUtils::StrCmp(pPipelineData->pPipelineName, "InternalZSLYuv2JpegGPU"))))
    {
        ...
        UINT32          vendorTagOverrideScaleMeta  = 0;
        ...
        if (NULL != pMetadata)
        {
            result = pMetadata->SetTag("org.quic.camera.overrideIPEScaleProfile",
                                       "OverrideIPEScaleProfile", &overrideScaleTag, 1);
            ...
            {
                CHX_LOG_INFO("OverrideIPEScaleProfile is set with value %d, vendorTagIndex %u",
                             overrideScaleTag, vendorTagOverrideScaleMeta);
            }
        }
        else
        {
    ...
    else
    {
        ...
        for (UINT32 stream = 0; stream < numStreams; stream++)
        {
            if (UsecaseSelector::IsRawStream(reinterpret_cast<camera3_stream_t*>(ppChiStreams[stream])))
            {
                ...
                {
                    RDIStream.format = ChiStreamFormatRaw16;
                    RDIStream.streamParams.bpp = 16;
                }
                else if (ppChiStreams[stream]->format == ChiStreamFormatRaw12)
                {
                    RDIStream.format = ChiStreamFormatRaw12;
                    RDIStream.streamParams.bpp = 12;
                }
            }
        }
            ...
            case ChiFeature2Type::B2Y:
                if (pNegotiationInfo->pLogicalCameraInfo->numPhysicalCameras == 1)
                {
                    if (FALSE == pNegotiationInfo->pFeatureInstanceId->flags.isNZSLSnapshot)
                    {
                        ...
                        {
                            pBayer2YuvInputStream->format           = RDIStream.format;

                            // configure input stream
                            pBayer2YuvInputStream->width            = RDIStream.width;
                            pBayer2YuvInputStream->height           = RDIStream.height;
                            pBayer2YuvInputStream->streamParams.bpp = RDIStream.streamParams.bpp;
                    ...
                    if (NULL != pBayer2YuvOutputStream)
                    {
                        ...
                        if (NULL != ppChiStreams)
                        {
                            for (UINT i = 0; i < numStreams; i++)
                            {
                                ...
                                {
                                    pBayer2YuvOutputStream->width         = ppChiStreams[i]->width;
                                    pBayer2YuvOutputStream->height        = ppChiStreams[i]->height;
                                    pBayer2YuvOutputStream->grallocUsage |= ppChiStreams[i]->grallocUsage;
                ...
                if (pNegotiationInfo->pLogicalCameraInfo->numPhysicalCameras == 1)
                {
                    if (FALSE == pNegotiationInfo->pFeatureInstanceId->flags.isNZSLSnapshot)
                    {
                        ...
                        if (NULL != pBPSInputStream)
                        {
                            ...
                            pBPSInputStream->format = RDIStream.format;
