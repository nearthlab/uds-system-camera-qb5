...
typedef struct {
    ...
    uint16_t    idle_peri_num;      /* number of addresses that designate IDLE period */
    uint16_t    idle_peri_adr[4];   /* addresses that designate IDLE period */
    uint16_t    idle_peri_val;      /* IDLE period */
} tl_dev_rom_exp_prm;
...
typedef struct {
    ...
    uint16_t            *pup_data;          /* Power-Up Sequenece data */
} tl_dev_eeprom;
