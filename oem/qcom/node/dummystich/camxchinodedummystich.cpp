    ...
    CHINODEBUFFERHANDLE hInput1)
{
    ...
    if ((RawMIPI10 == hOutput->format.format) ||
        (RawMIPI12 == hOutput->format.format) ||
        (RawMIPI14 == hOutput->format.format) ||
        (RawPlain16LSB10bit == hOutput->format.format) ||
        (RawPlain16LSB12bit == hOutput->format.format) ||
        (RawPlain16LSB14bit == hOutput->format.format) ||
    ...
    else if ((RawMIPI10 == hOutput->format.format) ||
             (RawMIPI12 == hOutput->format.format) ||
             (RawMIPI14 == hOutput->format.format) ||
             (RawPlain16LSB10bit == hOutput->format.format) ||
             (RawPlain16LSB12bit == hOutput->format.format) ||
    ...
    UINT64 requestId)
{
    ...
    INT64  dimension[2];
    dimension[0]                = static_cast<INT64>(m_format.width);
    dimension[1]                = static_cast<INT64>(m_format.height);
    tagList[index]              = g_VendorTagList[index] + g_vendorTagBase;
    tagData[index].size         = sizeof(CHITAGDATA);
    tagData[index].requestId    = requestId;
